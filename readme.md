...
>
> 本项目内所有3D模型、纹理贴图、绑定动画及着色器代码等均由作者独立制作完成。项目中所呈现的部分角色**视觉概念、外观设计、服饰特征**等元素，其原始知识产权（包括但不限于商标权、人物形象权）完全归属于「**Project シンフォギア**」系列及其版权方。作者的创作仅属于 “同人/致敬性质的艺术再创作”，不主张对上述角色概念的任何所有权。若原IP版权方认为此项目中的任何视觉元素侵犯了其合法权益，本人将响应版权方的正式要求，立即将该仓库设为私有或删除相关引用资源。



# 0. 效果展示

TODO：集中放几张图展示各部分效果（GI、Shader、物理动画、足部IK）

# 1. 图形与渲染

## 1.1. 自定义GI方案（LPV + Ray Marching）

![alt text](images/gi_teaser.jpg)
...
![alt text](images/gi_lpv_and_rm.jpg)

使用随机采样+屏幕空间法线、深度引导的自适应低通滤波实现不同的粗糙度效果。使用高阶插值的方式消除线性插值采样LPV时的视觉artifact。为了避免漏光，在采样LPV的光照数据时使用体素化场景的遮挡信息对采样坐标进行clipping。由于多次采样性能开销较大，将采样次数等参数设计成可调节的形式，可以选择不同的视觉质量-性能设置。

![alt text](images/gi_sample_comp.jpg)
...
![alt text](images/gi_dyna.gif)

数据的维护使用instancing的策略，相同的物体只会在VRAM中存储一份，每一份副本仅存储instance数据（如副本在世界空间的的位姿、指向实际数据buffer的引用等）。

![alt text](images/gi_export_lpvdata.jpg)

## 1.2. Shader

实现了类动画风格、与GI联动的原理化NPR Shader：基本原理是在Lambert和GGX的基础上做Color Ramp，辅以部分美术主导的动态效果（如边缘光、根据法线方向调整物理参数等等）来实现根据环境光变化而同步变化的、包含漫反射与发丝的各向异性高光等效果的、动画风格的着色。为了与GI联动，在计算Lambert和GGX的“基础光照”时，需要同时考虑GI的影响。由于动画中的阴影分割并不与绝对亮度挂钩，不能直接使用LPV给出的亮度，而是首先计算LPV光照的一阶微分，据此确定场景中的主要光源方向，然后由此确定阴影的分割方式。

描边轮廓则使用经典的backface culling+法线方向膨胀的方法实现。表面的物理数据，如粗糙度、金属性、各向异性高光的切线方向等，主要存储在顶点属性中。为了方便美术资产制作，也开发了相应的管线工具。

角色以外的场景Shader：直接使用了PBR管线，主要特点是：为减轻美术资产制作负担（避免手动维护UV和逐物体的贴图）、减少材质和drawcall数量，使用顶点色+Triplanar Mapping实现不同的颜色和外观。同时，将多组texture整合到单一的texture array中，通过顶点数据指定要采样的texture，进一步减少材质数量，并且允许单一物体使用单一材质给mesh的不同部分使用不同质感的贴图。

![alt text](images/shader.jpg)

# 2. 底层优化、物理模拟与引擎客制化

## 2.1. 地图数据结构、高效物理检测与寻路系统

![alt text](images/pathfinding.jpg)

### 2.1.1. 背景

战棋游戏的地图是网格结构，Godot引擎本身针对寻路、物理检测等任务提供的通用解决方案无法高效地处理这一逻辑。而游戏设计上决定赋予角色以更强的机动性，使其能够沿抛物线在网格地图中进行长距离跳跃、从高处下落等等。同时，游戏需要兼容不同体型（即占地面积是不同格子数量）的角色。这些因素使得可达位置搜索与寻路的难度随之增高，脚本语言和通用解决方案已经难以满足任务的性能需求。因此，使用C++编写了更加高效、紧凑的数据结构及相应的射线检测和寻路算法等，在脚本层面仅暴露高层次的API，提升性能。
...
## 2.2. 程序化动画（PBD/Verlet物理模拟、足部IK自动接地等）

※ 游戏内物理模拟为60fps，以下图片帧率受动图格式和体积限制，不如游戏内流畅

![alt text](images/phys.gif)

### 2.2.1. 背景

角色造型中带有丝带、裙摆等元素，在DCC软件中预先制作相关的物理效果低效而且难以在游戏中对不同的环境、运动状态等作出响应。因此需要使用基于物理模拟的方案来实现程序化生成的动画。而Godot原生的解决方案具备许多局限性：一方面，通用物理引擎较为臃肿，且只能基于mesh进行模拟，效果收到mesh本身的拓扑结构影响，并且与DCC软件中的骨骼动画配合相对麻烦；另一方面，Godot提供的、专用于角色物理动画的SpringBoneSimulator仅能支持丝带状的物体，无法应用于较为复杂的布料形状（如披肩等），并且具有未能正确实现角色移动造成的惯性影响、对于长度较长的丝带等结构容易产生不稳定结果、难以获得灵活、轻便质感等局限。因此自行开发了基于PBD（Position-Based Dynamics）/Verlet Integration的物理模拟方案，通过添加edge-collision、使用非胡克弹簧等等一系列措施满足了项目的动画物理需求。

另外，项目的3D场景中可能包含不平整的地面、阶梯等元素，且角色的朝向是任意的。固定的动画难以达到理想的视觉效果，并且制作费时。因此开发了自动根据朝向变化与地形碰撞信息动态调整角色腿部和躯干动作的算法。

### 2.2.2. 技术细节

物理模拟方案基于PBD/Verlet，直接在位置上处理碰撞约束；以骨骼而非mesh顶点作为模拟的单元，降低计算量，并且使得模拟精度与mesh复杂度解耦，提供更大的资产制作自由度。使用弹簧质点模型建模布料和丝带。
...
- 各个角色的物理单独计算，可以以角色为单位并行计算。单个角色单帧的物理逻辑通常仅需0.2~0.4ms即可完成计算，在多核处理器上能够支持大量角色的复杂物理设计。
